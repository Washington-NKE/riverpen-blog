...
const CommentsForm = ({ slug }: CommentsFormProps) => {
    ...
    submitComment(commentObj)
      .then((res) => {
        if (res && res.success) {
          ...
          setError(true);
          setErrorMessage(res.message || 'Failed to submit comment. Please try again later.');
        }
      })
      .catch((err) => {
  ...
  return (
    <div className="bg-white shadow-lg rounded-lg p-8 pb-12 mb-8">
      ...
      <div className="mt-8">
        <button type="button" onClick={handlePostSubmission} className="transition duration-500 ease hover:bg-indigo-900 inline-block bg-gradient-to-r from-[#cd19ff] to-[#e000ac] text-lg font-medium rounded-full text-white px-8 py-3 cursor-pointer">Post Comment</button>
